...
def validate_token(func):
    ...
    def wrap(self, *args, **kwargs):
        ...
        if not access_token:
            return json.dumps({'msg':"access_token_not_found missing access token in request header", status: 401})
        ...
        if access_token_data.find_or_create_token(user_id=access_token_data.user_id.id) != access_token:
            return json.dumps({'msg':"access_token token seems to have expired or invalid", status: 401})
